    ...
    <div class="col-xl-12">
        <div class="card">
            <div class="card-header align-items-center d-flex">
                <h4 class="card-title mb-0 flex-grow-1">Periode</h4>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="row mb-3">
                            <div class="col-lg">
                                ...
                                <select name="year_id" id="year_id" class="form-select form-control">
                                    @foreach($years as $year)
                                    <option value="{{ $year->id }}">{{ $year->name }}</option>
                                    @endforeach
                                </select>
                                <div class="text-danger text-error" id="year_id_error">
                                    *error
                                </div>
                            </div>
                        </div>
...
<script type="text/javascript">
    ...
    <?php if (can_access($permissions, $key_, 'add') || can_access($permissions, $key_, 'edit')) { ?>
        ...
        function save() {
            ...
            if (save_method == 'add') {
                url = "{{ url('/admin/period/add') }}";
            } else {
                url = "{{ url('/admin/period/update') }}";
            }
    ...
    <?php if (can_access($permissions, $key_, 'edit')) { ?>
        ...
        function togglePeriod(el, id) {
            const status = el.checked ? 1 : 0;

            $.ajax({
                url: "{{ url('/admin/period/toggle') }}",
                type: "POST",
                data: {
                    id: id,
                    status: status,
